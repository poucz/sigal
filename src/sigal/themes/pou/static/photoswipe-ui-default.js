...
(function (root, factory) {
	...
	var PhotoSwipeUI_Default = function (pswp, framework) {
		...
		var _overlayUIUpdated = false,
			...
			_defaultUIOptions = {
				...
				shareButtons: [
					...
					{id: 'delete', label: 'Delete image', url: '{{raw_image_url}}'}
				],
		...
		function updateTags(){
			...
			directory=directory.replace(/.*localhost[^\/]*\/([^#]*).*/g,"$1")
			
			var request = new Request( "getTags?fileName="+directory+pswp.currItem.src);
			fetch(request) 
			.then((response) => response.text())
			.then((text) => {
				document.getElementById("pswp__tags").innerHTML = text
			});
